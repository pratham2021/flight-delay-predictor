...
Machine learning model don't understand plain text, but they do understand numbers. We wanted to assign numerical categories for each origin, destination, and route, and carrier with respective TargetEncoders
...
are also important factors when making a prediction. These didn't require any TargetEncoders. We had to group by the FL_DATE, DEPARTURE_HOUR, ORIGIN/ROUTE/DEST for each ORIGIN/ROUTE/DEST and get the respective counts for each column (ORIGIN_HOURLY_FLIGHT, ROUTE_HOURLY_FLIGHTS, DEST_HOURLY_FLIGHTS).

Historical information regarding airline delay reputation, airport delay reputation, and route delay reputation is also important to look at. We head to group by figure out the delay rate for airline, origin airport, destination airport, and route. and create new corresponding columns as such where we mapped the each one of (ORIGIN, DEST, ROUTE) to its respective delay rate.
